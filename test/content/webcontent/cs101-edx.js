...
function loadImage(filename) {
  ...
  else if (window.globalPathPrefix2 && filename.length>0 && filename[0] != '/') {
    filename = window.globalPathPrefix2 + filename;
  }
...
function preflightCheck(code) {
  ...
  while ((result = reIf1.exec(code)) != null) {
    ...
    if (check) {
      ...
      if (!check.match(/^\s*\(/)) return "<b>if</b> should be immediately followed by <b>(</b>test<b>)</b> but found:<br></br>" + found;
      if (!check.match(/\)\s*\{\s*($|\/\/)/)) return "<b>if test</b> should end with <b>)</b> but found:<br></br>" + found;
      if (check.match(/[^<>=!]\=[^=]/)) return "<b>if test</b> should not contain single = (use == &lt;= &gt;=), but found:<br></br>" + found;
      if (check.match(/[^&]&[^&]/)) return "<b>if test</b> should not contain single & (use double &&), but found:<br></br>" + found;
      if (check.match(/[^|]\|[^|]/)) return "<b>if test</b> should not contain single | (use double ||), but found:<br></br>" + found;
    }
    else {
      // Experiment: flag that we cannot find { within 4 lines of the opening if as
      // an error.
      return "<b>if-statement</b> cannot find end of line <br></br>left curly brace <b>{</b> <br></br>Form should be " +
             "<br></br><b>if (test) {</b><br></br>&nbsp;..code..<br></br>but found:" + result[0];  // note using result from initial match.
    }
  }
  ...
  while ((result = reFor.exec(code)) != null) {
    ...
    var line = result[1];
    if (!line.match(/^\s*\(/)) return "<b>for</b> should be immediately followed by <b>(</b> but found:<br></br>" + result[0];
    if (!line.match(/\{\s*($|\/\/)/)) return "<b>for</b> line should end with curly brace <b>{</b> but found:<br></br>" + result[0];
    if (!line.match(/\)\s*\{\s*($|\/\/)/)) return "<b>for</b> should be followed by <b>(</b>part: whole<b>)</b> but found:<br></br>" + result[0];
  }
...
// edx-mod
// Previous: hard code edx static path
//window.globalPathPrefix = "/asset-v1:OLI+CS101+2015+type@asset+block@";
//window.globalPathPrefix = "/c4x/OLI/CS101/asset/";

// Somewhat better: figure out our asset url looking at special 1x1.png image embedded on every page
// We use globalPathPrefix2 so as not to conflict with old globalPathPrefix inside XBlock
if (!window.globalPathPrefix2) {
  var images = document.getElementsByClassName("hackhackhack");
  if (images) {
    var src = images[0].src;
    var start = src.indexOf("/", 12); // start 12 chars in, avoiding leading //
    var end = src.lastIndexOf("/");
    window.globalPathPrefix2 = src.substring(start, end+1);
  }
}
