      ...
      return o;
		...
		$$(".initiator").makeDraggable(
			...
			onDrop: function(draggable, droppable)
			{
			    ...
			    if(instance.isDisabled || draggable.getParent().get("input_ref")){
				return;
				}
		...
		this.currentState[partId] = value;
		this.currentState[value] = partId;

		if (this.isConvertedFromSWF) {
		...
		var droppable = this.viewPlaceholder.getElement('div[input_ref="' + id + '"]');
		var borderColor = isCorrect?"green":"red";
		...
		var selector = this.isConvertedFromSWF ? 'div[input_val]' : 'div[input_ref]'
		...
		var item = this.isConvertedFromSWF ? 'div[input_val="' + id + '"]' : 'div[input_ref="' + id + '"]';
