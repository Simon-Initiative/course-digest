			...
			if (stateObject) {
				...
				while (undoList.length > 0) {
					...
					case "modify":
						...
						break;
					case "bulkModify":
						for (var p in undo.propertiesChanged) {
							undo.fact[p] = undo.propertiesChanged[p];
						}
						this.modify(undo.fact, null, undo.recency);
						break;
					}
    ...
    instance: {
		...
		genUndoOperation: function(op, fact, optProperty) {
			...
			case 'bulkModify': 
				...
				optProperty.forEach(function(p){ret.propertiesChanged[p] = fact[p]})
				break;
			}
			return ret;
		},
...
exports.getMemory = (function () {
    ...
    return function _getMemory(entry, factHash, indexes, nodeName) {
        //entry === this nodes memory
		//factHash === the propagated context
		//indexes === ?? 
		if (nodeName) {
			console.log(nodeName+" getMemory: ");
        ...
        while (++i < l && rl) { //for each in indexes
            index = indexes[i];
...
declare({
    ...
    instance: {
        ...
        getMemory: function (tuple, nodeName) {
            var ret;
            if (!this.length) {
                ret = [];
            } else {
                ret = getMemory(this.tables, tuple.factHash, this.indexes, nodeName);
            }
            return ret;
        },
...
Memory.extend({
    ...
    instance: {
        ...
        getRightMemory: function (tuple, nodeName) {
            return this.getMemory(tuple, nodeName);
        }
    }
...
Super.extend({
    instance: {
		...
		//	console.log("node "+this.toString()+' assertLeft()');
			...
			var values = this.rightTuples.getRightMemory(context, this.toString()),
