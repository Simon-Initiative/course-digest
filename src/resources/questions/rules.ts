...
const makeBtwRule = (lesser: string, greater: string) =>
  andRules(
    orRules(gtRule(lesser), eqRule(lesser)),
    orRules(ltRule(greater), eqRule(greater))
  );
...
export const nbtwRule = (left: string, right: string) =>
  invertRule(btwRule(left, right));
...
export const makeRule = (
  ...
  throw new Error(
    'Could not make numeric rule for operator ' +
      operator +
      ' and input ' +
      input
  );
...
export const matchListRule = (all: string[], toMatch: string[]) => {
  ...
  return andRules(
    ...toMatch
      .map(matchRule)
      .concat(notToMatch.map((id: any) => invertRule(matchRule(id))))
  );
};

// Match the `ordered` list in exactly that order
export const matchInOrderRule = (ordered: string[]) =>
  `input like {${ordered.join(' ')}}`;
